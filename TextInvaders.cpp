...
// returns the shield index of which shield got hit, return NOT_IN_PLAY if nothing hit
...
int ResolveAlienCollision(AlienSwarm& alien, const Position& hitPositionInAliensArray);
...
void MovePlayer(const Game& game, Player& player, int dx)
{
	...
	{
		player.position.x = game.windowSize.width - player.spriteSize.width; // the right most the player can be
	}
	else if(player.position.x + dx < 0) // player is off the left
	{
		player.position.x = 0;
	}
	else
...
void PlayerShoot(Player& player)
{
	...
	{
		player.missile.y = player.position.y - 1; // one row above player
...
void DrawAliens(const AlienSwarm& aliens)
{
	...
	// draw 2 rows of 20 point aliens
	...
	// draw 2 rows of 10 point aliens
...
bool UpdateAliens(const Game& game, AlienSwarm& aliens, Player& player, Shield shields[], int numberOfShields)
{
	...
	if(!moveHorizontal)
	{
		...
		if(ShouldShootBomb(aliens))
		{
			if(numActiveCols > 0)
			{
				int numberOfShots = ((rand() %3) + 1) - aliens.numberOfBombsInPlay; // makes sure there are only 3 bombs in play
